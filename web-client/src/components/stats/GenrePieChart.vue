...
const PALETTE = [
  "#5b7c32", "#4a7fa5", "#b06a2a", "#7a4d9e", "#3a9a7a",
  "#b04848", "#3a7aaa", "#9a7a2a", "#5a8a5a", "#8a4a6a",
  "#4a6a9a", "#7a6a3a",
];

const MIN_SLICE_PERCENT = 1.5;
...
const slices = computed(() => {
  ...
  if (otherCount > 0) {
    main.push({ label: "Other", count: otherCount, pct: (otherCount / total) * 100, color: "#57534e" });
  }
  return main;
});
...
const CX = 100, CY = 100, R = 80, R_INNER = 38;
...
function slicePath(startDeg: number, endDeg: number): string {
  ...
  return [`M ${o.x} ${o.y}`, `A ${R} ${R} 0 ${large} 1 ${o2.x} ${o2.y}`, `L ${i2.x} ${i2.y}`, `A ${R_INNER} ${R_INNER} 0 ${large} 0 ${i1.x} ${i1.y}`, "Z"].join(" ");
}

interface SliceSvg { path: string; color: string; label: string; pct: number; count: number; midAngle: number; }
...
<template>
  <div v-if="total === 0" class="py-8 text-center text-xs text-stone-500">No tracks in library yet.</div>
  <div v-else class="flex flex-col items-center gap-3">
    <div class="relative shrink-0">
      <svg viewBox="0 0 200 200" class="h-44 w-44 drop-shadow-sm">
        ...
        <text x="100" y="96" text-anchor="middle" dominant-baseline="middle" class="select-none" fill="#d6d3d1" font-size="13" font-weight="600">
          {{ hovered !== null ? svgSlices[hovered].pct.toFixed(1) + "%" : total }}
        </text>
        <text x="100" y="111" text-anchor="middle" dominant-baseline="middle" class="select-none" fill="#78716c" font-size="8">
    ...
    <div class="w-full flex flex-col gap-1">
      ...
      >
        ...
        <span class="w-10 shrink-0 text-right text-[11px] tabular-nums text-stone-400">{{ s.pct.toFixed(1) }}%</span>
      </div>
    </div>
  </div>
</template>
