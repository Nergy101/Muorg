<template>
  ...
  <div
    ...
    class="relative flex h-16 shrink-0 items-center bg-surface shadow-lg"
  >
    ...
    <div class="flex min-w-0 flex-1 items-center self-stretch" @click="onBodyTap">
      ...
      <div class="min-w-0 flex-1 px-3">
        <MarqueeText :text="player.currentTrack.title ?? '—'" class="text-body-md text-on-surface" />
        <div class="truncate text-body-sm text-on-surface-variant">
          {{ player.currentTrack.artist ?? player.currentTrack.album_artist ?? "—" }}
        </div>
        <div v-if="player.sleepTimerActive" class="flex items-center gap-1 text-label-sm text-primary">
