...
const hovered = ref<number | null>(null); // 1–5 or 0 for unrated

const bars = computed(() => {
  const counts = [0, 0, 0, 0, 0, 0]; // index 0 = unrated, 1–5 = stars
  ...
  <div v-else class="flex flex-col gap-3">
      ...
      >
        ...
        <div class="relative min-w-0 w-full flex-1 flex flex-col justify-end">
          <div
            ...
            :class="bar.stars === 0
              ? (hovered === bar.stars ? 'bg-stone-500' : 'bg-stone-600')
              : (hovered === bar.stars ? 'bg-amber-400' : 'bg-amber-500')"
    ...
    <div class="flex items-start gap-2">
      <div
        ...
        :class="[
          bar.stars === 0 ? 'text-[9px] text-stone-500' : 'text-[10px] text-amber-500/70',
          hovered === bar.stars ? (bar.stars === 0 ? '!text-stone-300' : '!text-amber-400') : ''
        ]"
      >
        {{ bar.label }}
      </div>
    </div>
    <!-- Hovered detail — always reserve space to avoid layout shift -->
    <p class="text-center text-xs text-stone-400 transition-opacity duration-100" :class="hovered !== null ? 'opacity-100' : 'opacity-0'">
      <template v-if="hovered === 0">
        <span class="font-medium text-stone-200">Unrated</span>
        — {{ bars[5].count }} tracks ({{ bars[5].pct }}%)
      </template>
      <template v-else-if="hovered !== null">
        <span class="font-medium text-amber-400">{{ hovered }}★</span>
        — {{ bars.find(b => b.stars === hovered)?.count }} tracks
        ({{ bars.find(b => b.stars === hovered)?.pct }}%)
      </template>
